...
export default function TradeRoomPage() {
  ...
  return <section className="content-section trade-room-page">
    ...
    <section className="trade-member-status panel"><h2>거래 완료 상태</h2>{room.members.map((member) => <span key={member.userId}>{member.nickname} · {member.completedAt ? '완료' : '진행 중'}</span>)}</section>
    <div className="message-list" aria-live="polite">{messages.map((message) => <article className={`chat-message ${message.senderId === user?.id ? 'mine' : ''}`} key={message.id || message.clientMessageId}>
    ...
    {completed ? <p className="readonly-message">완료된 거래는 대화 내용을 읽을 수만 있습니다.</p> : <>
      <form className="message-form" onSubmit={submit}><label className="sr-only" htmlFor="trade-message">메시지 입력</label><textarea id="trade-message" value={content} maxLength={1000} onChange={(event) => setContent(event.target.value)} placeholder="메시지를 입력하세요" /><button className="primary-button" type="submit">전송</button></form>
