...
export default function MatchesPage() {
  ...
  return (
    <section className="content-section matches-page">
      <header className="page-heading"><div><span className="eyebrow">MATCH CANDIDATES</span><h1>내 매칭 후보</h1></div><p>가중치가 높은 교환 경로부터 보여드려요.</p></header>
      ...
      <div className="match-list">{ordered.map((match) => (
        <article className="match-card" data-testid="match-card" key={match.id}>
          <header><div><span className="eyebrow">{typeLabels[match.matchType] || match.matchType}</span><div className="participant-list">{match.participants.map((participant) => <span key={participant.userId}>{participant.nickname}</span>)}</div></div><strong className="score-badge" aria-label="매칭 점수">{match.score}점</strong></header>
